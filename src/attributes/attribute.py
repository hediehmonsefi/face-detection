...
class Attribute:
    ...
    def get(self, img, face):
        """
        The actual method to get the prediction for a specific attribute.
...
#         pred = self.session.run(self.output_names, {self.input_name: face_crop})
#         pred = np.squeeze(pred)
#         if self.taskname == 'beard':
#             beard = int(np.round(pred))
#             face['beard'] = beard
#             return beard
#         else:
#             return pred
...
#         face_crop = np.expand_dims(face_crop, axis=0)
#         face_crop = face_crop.astype(np.float32)
#         pred = self.session.run(self.output_names, {self.input_name: face_crop})[0][0]
#         if self.taskname == 'mask':
#             mask = np.argmax(pred)
#             face['mask'] = mask
#             return mask
#         else:
